/**
 * Integration test for the message pipeline.
 *
 ...
 *
 * Optional env vars:
 *   INTEGRATION_RUNTIME_COMMAND — runtime command (default: "echo")
 *   INTEGRATION_TIMEOUT_MS      — max wait for completion (default: 10000)
 *
 * Run:
 *   pnpm --filter @telegramable/core test:integration
 */
...
const RUNTIME_COMMAND = process.env.INTEGRATION_RUNTIME_COMMAND ?? "echo";
const TIMEOUT_MS = Number(process.env.INTEGRATION_TIMEOUT_MS) || 10_000;
...
test("Integration: Telegram message flows through the full pipeline", {
